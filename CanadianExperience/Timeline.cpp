...
void CTimeline::Save(std::shared_ptr<xmlnode::CXmlNode> root)
{
	...
	root->SetAttribute(L"endttime", mEndTime);
    for (auto channel : mChannels)
    {
...
void CTimeline::Load(std::shared_ptr<xmlnode::CXmlNode> root)
{
	...
	mStartTime = root->GetAttributeIntValue(L"starttime", 10000);
	mEndTime = root->GetAttributeIntValue(L"endttime", 0);

	mMachineDrawable->SetEndTime(mEndTime);
	mMachineDrawable->SetStartTime(mStartTime);
...
/** \brief Set the timeduration
	...
	*/
void CTimeline::SetTimeDuration(int starttime, int endtime)
{
